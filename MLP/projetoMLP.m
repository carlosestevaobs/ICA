    ...
    for i = 1:epoca    
        ...
        %% sentido direto
           ...
           %% Atualização dos pesos da camada de entrada        
            ...
            ErroMSETreino(i,:) = immse(SaidaCalculada(:,j), CamadaSaida(:,j));
        end
        % fim da iteração
        ErroTotal(:,i) = erro;
    end
...
%% Popular do treinamento: matriz de confusão, erro quadrático médio e dados
...
for i=1:size(DadosTeste,2)
     ...
     ErroMSETeste(i,:) = immse(SaidaTeste(:,i), CamadaSaidaT(:,i));
end
erro = CamadaSaidaT - SaidaTeste;
erroTotalTeste = sum((erro(:,1:i).^2),1);

%% Popular do teste: matriz de confusão, erro quadrático médio e dados
